...
void BankCardStructure::addCard(BankCard&& card) {
    cards.emplace_back(std::move(card));
}
...
std::optional<std::tm> BankCardStructure::getValidityPeriodByOwnerNumber(const std::string& ownerNumber) const {
    for (const auto& card : cards) {
        if (card.getNumberOwner() == ownerNumber) {
            return card.getValidityPeriod();
        }
    }
    return std::nullopt; // Возвращаем std::nullopt, если карта не найдена
}

void BankCardStructure::sameNames(const BankCard& firstCard, const BankCard& secondCard) {
    if (firstCard.getNameCard() == secondCard.getNameCard()) {
        std::cout << "The cards have the same names.\n\n";
    }
    else {
        std::cout << "The cards have NOT the same names.\n\n";
